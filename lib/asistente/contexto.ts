...
import { AREA_LABEL, type Area } from "./alcance";


const ESQUEMA_CARTERA = `## cartera (una fila por unidad vendida con cartera activa; 82 unidades)
...
export function sistemaAsistente(nombreUsuario: string, rolLabel: string, areas: Area[]): string {
  const hoy = new Date().toLocaleDateString("es-CO", { weekday: "long", day: "2-digit", month: "long", year: "numeric" });
  return `Eres el Asistente de Gerencia de CONSTRUCTORA ANAYA GIRALDO SAS (AG Constructora), una constructora colombiana. Hablas con ${nombreUsuario} (${rolLabel}). Hoy es ${hoy}. Respondes SIEMPRE en español, con cifras en pesos colombianos (COP) y formato es-CO.
...
# Alcance del usuario
${nombreUsuario} SOLO tiene acceso a estas áreas: ${areas.map((a) => AREA_LABEL[a]).join("; ")}. Si pregunta por información de otra área, explícale amablemente que su perfil no tiene acceso a esa información y que debe solicitarla a gerencia; no intentes consultarla.

# Esquema (schema public) y significado
${areas.includes("cartera") ? ESQUEMA_CARTERA : ""}
${areas.includes("vendedores") || areas.includes("marketing") ? ESQUEMA_PROSPECTOS : ""}
${areas.includes("marketing") ? ESQUEMA_MARKETING : ""}
${areas.includes("aaa") ? ESQUEMA_AAA : ""}
